        ...
        public function mandar_email_usuario_suscritos($post_id)
        {
            if(($_POST['post_status'] == 'publish') && ( $_POST['original_post_status'] != 'publish'))
            {
                ...
                foreach ($comprobar_activo->posts as $key=> $email){
                    ...
                    //Sacamos el email activo
                    $id = $email->ID;
                    $email_suscriptor = get_post_meta($id, '_email', true);
                    
                    //mandamos email para el suscriptor
                    
                    ob_start();
                    require_once 'views/plantilla/email.php';
                    $mensaje = ob_get_contents();
                    ob_end_clean();
                    
                    wp_mail($email_suscriptor, "Novedades de " .$author->display_name, $mensaje, 'Content-type: text/html');
