...
if (!class_exists('WordPressSettingsFramework')) {
    ...
    class WordPressSettingsFramework {
        ...
        function __construct($settings_file, $option_group = '') {
            ...
            add_action('wp_ajax_importar', array($this, 'insertar_emails_antiguos'));
            add_action('wp_ajax_nopriv_importar', array($this, 'insertar_emails_antiguos'));
        }

        public function insertar_emails_antiguos() {
            
            global $wpdb;

            $dato = json_decode(stripslashes($_POST['data']));
            ...
            $nombre_tabla = $wpdb->prefix . "suscriptflech";
        ...
        public function lista_emails_anteriores() {

            global $wpdb;
            $nombre_tabla = $wpdb->prefix . 'suscriptflech';

            $datos = $wpdb->get_results("select email from " . $nombre_tabla);
            ...
            if (empty($datos)) {
                ...
                <div class="postbox">
                <h3><span style="margin-left: 1em;">  Listado Emails <span></h3>
                <div class="inside">
                <table class="widefat">
                <tr>
                <td><?php echo 'No hay email para importar'; ?></td>
                </tr>
                </table>
                </div>
                </div>
                ...
                <div class="postbox">
                <h3><span style="margin-left: 1em;">  Listado Emails <span></h3>
                <div id="final" class="inside">
                <table class="widefat" id="fximporta">
                <tbody>
                <?php foreach ($datos as $dato): ?>
                <tr>
                <td id="tabla" class="row-title"><label for="tablecell"><?php echo $dato->email ?></label></td>
                </tr>
                <?php endforeach; ?>
                </tbody>
                </table><br>
                    <div class="cargadoando">
                        <span id="carga_importar" style="display:none">Importando correos un momento ......</span>
                    </div> 
...
if (!function_exists('get_options_page')) {
    ...
    function get_options_page($value) {
        ...
        $settings = wpsf_get_settings(plugin_dir_path(__FILE__) . 'admin/includes/options/settings/settings-general.php');

        if (isset($settings['settingsgeneral_general_' . $value]))
            return $settings['settingsgeneral_general_' . $value];

        if (isset($settings['settingsgeneral_plantilla_' . $value]))
            return $settings['settingsgeneral_plantilla_' . $value];
